...
import { CLASS_POSTS, CREATE_CLASS_POST } from '../graphql'
import { useAuth } from '../contexts/AuthContext'
import Loading from './Loading'
import { APP_URL } from '../constants'

const CreateClassPostForm = ({ classId, addNewPostAndTag }) => {
  const { user } = useAuth()
  ...
  const [createClassPost, { loading: creatingPost }] = useMutation(
    CREATE_CLASS_POST,
    {
      ...
      update(cache, { data: { createClassPost } }) {
        const { classPosts } = cache.readQuery({
          query: CLASS_POSTS,
        ...
        cache.writeQuery({
          ...
          variables: { id: classId },
          data: {
            classPosts: [createClassPost, ...classPosts],
          },
        })
